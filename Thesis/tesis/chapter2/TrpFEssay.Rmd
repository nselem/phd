...
par(new=TRUE)
legend("right", legend = unique(PRAassay2.m$variable), col = unique(PRAassay2.m$variable), pch=19)
...
plot(PRAassay2Time.m$Time, PRAassay2Time.m$value, col=PRAassay2Time.m$variable, pch=19, xlim=c(cuttime-50,1000),bty='L')
...
par(xpd = TRUE)
slopeText2="m ="
slopeText<-paste(slopeText2,rev(round(V0[-1],digits=4)),rev(names(V0[-1])))
legend(800,200, legend = slopeText, col = rev(unique(PRAassay2Time.m$variable)), pch=19,box.lwd=0, bg="white")

## get relevant velocities
## get correspondent concentrations

## interpolar michaelis-menden
library(drc) # for fitting Michaelis Menten model
library(ggplot2) # for drawing
mm <- structure(list(S = c(3.6, 1.8, 0.9, 0.45, 0.225, 0.1125, 3.6, 1.8, 0.9, 0.45, 0.225, 0.1125, 3.6, 1.8, 0.9, 0.45, 0.225, 0.1125, 0), v = c(0.004407692, 0.004192308, 0.003553846, 0.002576923, 
...
ggplot(mm, aes(x = S, y = v)) + theme_bw() + xlab("Concentration [mM]") + ylab("Speed [dE/min]") + ggtitle("Michaelis-Menten kinetics") +  geom_point(alpha = 0.5) + geom_line(data = mml, aes(x = S, y = v), colour = "red")
