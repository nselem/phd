...
CoelicolortableExp$Org<-"Actinobacteria"
LividanstableExp$Org<-"Actinobacteria"
...
tableDistribution <- read.csv("Figuras/Coeli100", header=TRUE, row.names=1,sep="\t")   ### Enzyme BGC Function from MiBIG coelicolor
LivitableDistribution  <- read.csv("Figuras/Livi100",header=TRUE, row.names=1,sep="\t")   ### Enzyme BGC Function from MiBIG coelicolor
tableCentralDistribution <- read.csv("Figuras/Central100", header=TRUE, row.names=1,sep="\t")  ### Enzyme Subsystem Function from central metabolism  

tableDistribution[1,Enzyme_19]
...
EvoMining previous version genome-DB comprises 230 genomes from Actinobacteria, a phylum known by being a source of natural products biosynthetic gene clusters. EvoMining genomic model takes as postulates that secondary metbolism BGCs often includes enzymes that come from enzyme families that have been expanded and recruited to perform new metabolic functions. These assumptions are satisfied by other bacteria that posses expansions of enzyme families from central metabolism, as a consecuence EvoMining can be applied to other bacterial taxonomical groups beside Actinobacteria. Cyanobacteria for example is another phylum with known presence of biosynthetic gene clusters.
...
as genomic-DB 230 genomes with 106 central families as the corresponding central-DB. 
To incorporate other genomic databases to the pipeline it was important to determine wether or not other taxonomic groups actually posses expansions of families devoted to central metabolism. Nevertheless before exploring expansions from central enzyme families on other taxa beside Actinobacteria it is needed a previous identification of central families themselves on the new genomic databases. A common set of central enzyme families was determined for *Actinobacteria*, *Cyanobacteria*, *Pseudomonas* and even *Archaeas* by finding  best bidirectional hits (BBH) of original 339 seeds belonging to the 106 central Actinobacteria enzyme families on selected genomes. To avoid missing some BBH due to genome quality selected genomes were chosen to be on one contig. As an outcome of this process a database with 42 enzyme families common to this taxonomical groups was assembled, and *Actinobacteria*, *Cyanobacteria*, *Pseudomonas* and *Archaeas* were explored in search of central enzyme families expansions. 

Selected genomes were BBH from 106 central enzyme families were searched are 1)Cyanobacteria phylum: * Cyanothece sp. ATCC 51142*, *Synechococcus sp. PCC 7002 CP000951.1*, *Synechocystis sp. PCC 6803 AP012205.1*, 2)Pseudomonas generus *Pseudomonas fluorescens pf0-1*,*Pseudomonas protegens Pf5*,*Pseudomonas syringae pv. actinidiae ICMP 18884 NZ_CM0027 51.1*,*Pseudomonas fulva 12-X NC_015556.1*
3) Archaea kingdom *Natronomonas pharaonis*,*Methanosarcina acetivorans*, *Sulfolobus solfataricus* and *Nanoarchaeum equitans Kin4-M*.


This results showed that expansion of a central family is calde specific, a family can be expanded on Actinobacteria but not on Cyanobacteria, even more a family can be part of the core of Actinobacteria but not exist on Archaea.
...
![GenomicDatabases](Figuras/CentralDB.svg)  <!--Poner juntos cyanos Archea, marcar por forma orden favorito -->
![BackwardEvomining](Figuras/EvoBackward2.svg)  
![EnzymeFamilies](GenomicDiversity.svg)  
![TauD](Figuras/TauDpseudomonas2.svg)  
![Shell enzyme](Figuras/naturalDB.svg)  
