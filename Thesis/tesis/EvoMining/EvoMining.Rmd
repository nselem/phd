...
Selem-Mojica Nelly,  Martínez-Guerrero Christian , ...,  and Barona-Gómez Francisco   							  
...
Cyanos.Colors.Reduced<-CyanosColors[intersect(row.names(ArchaeasColors),row.names(CyanosColors)),]
Archaeas.Colors.Reduced<-ArchaeasColors[intersect(row.names(ArchaeasColors),row.names(CyanosColors)),]  

str(Archaeas.Colors.Reduced)  
str(Cyanos.Colors.Reduced)  
Todos.Colors<-rbind(Archaeas.Colors.Reduced,Cyanos.Colors.Reduced)   

#Todos.Colors$Red_Final<-Todos.Colors$FirstRED-Todos.Colors$CYAN_OVER_RED
#Todos.Colors$Cyan_Cyan<-Todos.Colors$CYAN -Todos.Colors$CYAN_OVER_RED
#Todos.Colors$Black<-Todos.Colors$TOTALTree-(Todos.Colors$CYAN+Todos.Colors$Red_Final+Todos.Colors$Azul_NP)  

Colors=Todos.Colors[names(Todos.Colors) %in% c("Red_Final","CYAN_OVER_RED","Cyan_Cyan","Black","Azul_NP","ORANGE","Kingdom")]

Cyanos.Colors.Reduced=Cyanos.Colors.Reduced[names(Cyanos.Colors.Reduced) %in% c("Family","Red_Final","CYAN_OVER_RED","Cyan_Cyan","Black","Azul_NP","ORANGE","Kingdom")]
Cyanos.Colors.Reduced<-Cyanos.Colors.Reduced[c(1,6,3,4,5,7,2,8)]
...
ggplot(Cyanos.Colors.Reduced.m, aes(x = Cyanos.Colors.Reduced.m$Family, y = Cyanos.Colors.Reduced.m$value,  fill = Cyanos.Colors.Reduced.m$variable)) +geom_bar(stat = "identity")+theme(plot.title = element_text(size = 14, face = "bold"), text = element_text(size = 12), axis.title = element_text(face="bold"), axis.text.x=element_text(angle = 90,size = 10), legend.position = "bottom")+
    scale_fill_manual(values=c( "#A901DB","#0000FF", "#FAAC58","#FF0000", "#58FAF4","#000000", "#E69F00"))


Colors.m<- melt(Todos.Colors,id = c("Family","Kingdom"))

##### Solo me falta graficar  
ggplot(Colors.m, aes(x = Colors.m$Family, y = Colors.m$value,  fill = Colors.m$variable)) +geom_bar(stat = "identity")+facet_grid(~Colors.m$Kingdom)+theme(plot.title = element_text(size = 14, face = "bold"), text = element_text(size = 12), axis.title = element_text(face="bold"), axis.text.x=element_text(angle = 90,size = 10), legend.position = "bottom")+
    scale_fill_manual(values=c( "#A901DB","#0000FF", "#FAAC58","#FF0000", "#58FAF4","#000000", "#E69F00"))
...
```{r CoelicolorMiBig, warnings = FALSE, message = FALSE,echo=FALSE, results='hide'}
