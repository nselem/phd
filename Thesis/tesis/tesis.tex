...
% Everything below added by CII
  ...
  Dentro de MIBiG hay 231 BGC (12.7\%) clasificados como ``otros'', que de
  ...
  {[}\protect\hyperlink{ref-engel_glutamate_2014}{145}{]}{]} y en la
  síntesis de cadena ramificada aminoácidos
  ...
  \tiny   \textbar{}Fuente\textbar{}Familia\textbar{}HisA
  ...
  {[}\protect\hyperlink{ref-henn-sax_two_2002}{166}{]}\textbar{}\\
  \textbar{} \emph{Escherichia coli}
  \textbar{}TrpF\textbar{}-\textbar{}-\textbar{}-\textbar{}-\textbar{}0\textbar{}12.2\textbar{}34.5\textbar{}2.82\textbar{}Sterner
  {[}\protect\hyperlink{ref-sterner_phosphoribosyl_1996}{167}{]}\textbar{}\\
  ...
  Due {[}\protect\hyperlink{ref-due_bisubstrate_2011}{168}{]}\textbar{}\\
  ...
  {[}\protect\hyperlink{ref-noda-garcia_identification_2010}{169}{]}\textbar{}\\
  \textbar{} \emph{Streptomyces coelicolor}
  \textbar{}PriA\textbar{}-\textbar{}-\textbar{}3.6 ± 0.7 \textbar{}1.3 ±
  0.2 \textbar{}0.4\textbar{}5.0 ± 0.08 \textbar{}3.4 ± 0.09
  \textbar{}0.7\textbar{}Noda
  ...
  {[}\protect\hyperlink{ref-gao_biosynthesis_2017}{170}{]}. La
  pentostatina es un atibiótico nucleosídico derivado de adenosina, cuyo
  cluster es llamado \emph{ada} . La PriA del cluster \emph{ada} es
  llamada \emph{adaK} y sí parece participar del cluster, ya que el
  sustrato muestra una isomerización sobre un sustrato con una anillo de 5
  carbonos, dos OH, un oxígeno y un grupo fosfato. Esta isomerización es
  muy parecida a la que realiza PriA sobre ProFAR y PRA. Así pues
  habríamos encontrado una nueva promiscuidad de PriA. Es relevante
  mencionar que la mutante de PriA no suprime la producción de este
  antibiótico en \emph{Actinomadura} sp. ATCC 39365, por lo que los
  autores especulan que otra enzima podría estar realizando la
  isomerización. \emph{Actinomadura} no posee una copia de TrpF que sería
  el candidato inmediato.
  ...
  \begin{figure}[h!tbp]
  \centering
  \includegraphics[angle = 0,scale = 0.6]{chapter4/LianetFiguras/SolocirculosPRA_PRO_RUTAS_10_1_r002.png}
  ...
  no es estrictamente creciente, si no que suele haber pasos donde alguna
  ...
  {[}\protect\hyperlink{ref-marks_protein_2011}{171}{]}. En esta sección
  ...
  {[}\protect\hyperlink{ref-marks_protein_2011}{171}{]}, para poder
  ...
  {[}\protect\hyperlink{ref-hopf_evcouplings_2019}{172}{]} que comprende
  ...
  del alineamiento, diferenciando entre secuencias PriA, subHisA, PriB y
  subTrpF.
  ...
  como altamente partícipes en la covariación. En la siguiente tabla se
  muestra en la primera columna los aminoácidos variados en la sección
  anterior, y en la segunda columna el aminoácido correspondiente en la
  secuencia de \emph{Streptomyces coelicolor}.
  ...
  este capítulo, PriA participa en la síntesis del antibiótico
  pentostaina. Tomando este ejmplo como inspiración, se colectaron en la
  literatura sustratos parecidos químicamente a ProFAR y PRA. En esta
  sección se buscará mostrar sustratos parecidos a los nativos con la idea
  de posteriormente probarlo en copias seleecionadas de PriA provenientes
  ...
  celulosa {[}\protect\hyperlink{ref-chertkov_complete_2011}{173}{]}.
  ...
  {[}\protect\hyperlink{ref-pukall_complete_2009}{174}{]}.
  ...
  \KeywordTok{kable}\NormalTok{(docking,  }
        \DataTypeTok{caption =} \StringTok{"Enzymes docking }\CharTok{\textbackslash{}\textbackslash{}}\StringTok{label\{tab:docking\}"}\NormalTok{,}
        \DataTypeTok{caption.short =} \StringTok{"Enzymes docking "}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{#First I saved file as scv tablar separated    }
  \CommentTok{#perl -p -i -e 's/,/\textbackslash{}t/g' Pra_scoe1.csv   }
  \CommentTok{#An cut it to obtain just data  }
  \CommentTok{#tail -n +39 Pra_scoe1.csv | head -n-3 > Pra_scoe1.data  }
  \CommentTok{#perl -p -i -e 's/\textbackslash{}sNr\textbackslash{}.|\textbackslash{}s\textbackslash{}[\textbackslash{}w*\textbackslash{}]|\textbackslash{}.\textbackslash{}s\textbackslash{}[.*\textbackslash{}]//g' Pra_scoe1.data}
  \CommentTok{#tablePRAPRO <- read.table(s"chapter2/cineticas/Pra_scoe1.data",header=TRUE, sep="\textbackslash{}t") }
  \NormalTok{tablePRO <-}\StringTok{ }\KeywordTok{read.table}\NormalTok{(}\StringTok{"cineticas/18082017_PraProfarAbsorbanceCombined.data"}\NormalTok{,}\DataTypeTok{header=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{sep=}\StringTok{"}\CharTok{\textbackslash{}t}\StringTok{"}\NormalTok{) }
  \NormalTok{tablePRA <-}\StringTok{ }\KeywordTok{read.table}\NormalTok{(}\StringTok{"cineticas/18082017_PraProfarFluorCombined.data"}\NormalTok{,}\DataTypeTok{header=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{sep=}\StringTok{"}\CharTok{\textbackslash{}t}\StringTok{"}\NormalTok{) }
  \CommentTok{#tablePRA <- read.table("15082017_praOSCURO.data",header=TRUE, sep="\textbackslash{}t") }
  
  
  \NormalTok{## visualising full data}
  \NormalTok{tablePRO$Temp<-}\OtherTok{NULL}
  \NormalTok{tablePRO$Cycle<-}\OtherTok{NULL}
  
  \NormalTok{a<-tablePRO$Time[}\KeywordTok{c}\NormalTok{(}\OtherTok{TRUE}\NormalTok{,}\OtherTok{FALSE}\NormalTok{)]}
  \NormalTok{b<-tablePRO$Time[}\KeywordTok{c}\NormalTok{(}\OtherTok{TRUE}\NormalTok{,}\OtherTok{FALSE}\NormalTok{)]+}\FloatTok{19.15}
  
  \NormalTok{tablePRO$Time[}\KeywordTok{c}\NormalTok{(}\OtherTok{TRUE}\NormalTok{, }\OtherTok{FALSE}\NormalTok{)] <-}\StringTok{ }\NormalTok{a}
  \NormalTok{tablePRO$Time[}\KeywordTok{c}\NormalTok{(}\OtherTok{FALSE}\NormalTok{, }\OtherTok{TRUE}\NormalTok{)] <-}\StringTok{ }\NormalTok{b}
  \NormalTok{tablePRO$Time}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{tablePRO<-tablePRO[}\KeywordTok{grep}\NormalTok{(}\StringTok{"Pro|Time"}\NormalTok{, }\KeywordTok{colnames}\NormalTok{(tablePRO))]}
  \CommentTok{#tablePRA<-tablePRA[grep("Pra|Time", colnames(tablePRA))]}
  
  \NormalTok{PROassay.m <-}\StringTok{ }\KeywordTok{melt}\NormalTok{(tablePRO,}\DataTypeTok{id=}\StringTok{"Time"}\NormalTok{)}
  
  \KeywordTok{plot}\NormalTok{(PROassay.m$Time, PROassay.m$value, }\DataTypeTok{col=}\NormalTok{PROassay.m$variable,  }\DataTypeTok{xlab=}\StringTok{"Time [s]"}\NormalTok{,}\DataTypeTok{ylab=}\StringTok{"Absorbance"}\NormalTok{, }\DataTypeTok{pch=}\DecValTok{19}\NormalTok{,}\DataTypeTok{bty=}\StringTok{'L'}\NormalTok{)}
  \KeywordTok{par}\NormalTok{(}\DataTypeTok{xpd =} \OtherTok{TRUE}\NormalTok{)}
  \KeywordTok{legend}\NormalTok{(}\StringTok{"right"}\NormalTok{, }\DataTypeTok{legend =} \NormalTok{(}\KeywordTok{unique}\NormalTok{(PROassay.m$variable)), }\DataTypeTok{col =} \NormalTok{(}\KeywordTok{unique}\NormalTok{(PROassay.m$variable)),}\DataTypeTok{pch=}\DecValTok{19}\NormalTok{,}\DataTypeTok{bg=}\StringTok{"white"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{ggplot}\NormalTok{(PROassay.m, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \NormalTok{PROassay.m$Time, }\DataTypeTok{y =} \NormalTok{PROassay.m$value),}\DataTypeTok{color=}\StringTok{"variable"}\NormalTok{) +}\StringTok{ }\KeywordTok{theme_bw}\NormalTok{() }
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{qplot}\NormalTok{(Time,value, }\DataTypeTok{data =} \NormalTok{PROassay.m,}\DataTypeTok{colour=}\NormalTok{variable)+}\KeywordTok{theme_bw}\NormalTok{()+}\StringTok{ }\KeywordTok{theme}\NormalTok{(}\DataTypeTok{legend.position  =}\StringTok{"bottom"}\NormalTok{,}\DataTypeTok{legend.text=}\KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size=}\DecValTok{10}\NormalTok{),}\DataTypeTok{plot.title =} \KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size =} \DecValTok{14}\NormalTok{, }\DataTypeTok{face =} \StringTok{"bold"}\NormalTok{),}\DataTypeTok{text =} \KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size =} \DecValTok{12}\NormalTok{),}\DataTypeTok{axis.title =} \KeywordTok{element_text}\NormalTok{(}\DataTypeTok{face=}\StringTok{"bold"}\NormalTok{),}\DataTypeTok{axis.text.x=}\KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size =} \DecValTok{10}\NormalTok{),}\DataTypeTok{axis.text.y=}\KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size =} \DecValTok{7}\NormalTok{))+}\KeywordTok{facet_grid}\NormalTok{(PROassay.m$variable~.,}\DataTypeTok{scales=}\StringTok{"free"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Highlighting}[]
  \NormalTok{## visualising full data}
  \NormalTok{tablePRA$Temp<-}\OtherTok{NULL}
  \NormalTok{tablePRA$Cycle<-}\OtherTok{NULL}
  
  \NormalTok{a<-tablePRA$Time[}\KeywordTok{c}\NormalTok{(}\OtherTok{TRUE}\NormalTok{,}\OtherTok{FALSE}\NormalTok{)]}
  \NormalTok{b<-tablePRA$Time[}\KeywordTok{c}\NormalTok{(}\OtherTok{TRUE}\NormalTok{,}\OtherTok{FALSE}\NormalTok{)]+}\FloatTok{19.15}
  
  \NormalTok{tablePRA$Time[}\KeywordTok{c}\NormalTok{(}\OtherTok{TRUE}\NormalTok{, }\OtherTok{FALSE}\NormalTok{)] <-}\StringTok{ }\NormalTok{a}
  \NormalTok{tablePRA$Time[}\KeywordTok{c}\NormalTok{(}\OtherTok{FALSE}\NormalTok{, }\OtherTok{TRUE}\NormalTok{)] <-}\StringTok{ }\NormalTok{b}
  \NormalTok{tablePRA$Time}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{tablePRA<-tablePRA[}\KeywordTok{grep}\NormalTok{(}\StringTok{"Pra|Time"}\NormalTok{, }\KeywordTok{colnames}\NormalTok{(tablePRA))]}
  
  \NormalTok{PRAassay.m <-}\StringTok{ }\KeywordTok{melt}\NormalTok{(tablePRA,}\DataTypeTok{id=}\StringTok{"Time"}\NormalTok{)}
  \KeywordTok{plot}\NormalTok{(PRAassay.m$Time, PRAassay.m$value, }\DataTypeTok{col=}\NormalTok{PRAassay.m$variable,  }\DataTypeTok{xlab=}\StringTok{"Time [s]"}\NormalTok{,}\DataTypeTok{ylab=}\StringTok{"Fluorescence"}\NormalTok{, }\DataTypeTok{pch=}\DecValTok{19}\NormalTok{,}\DataTypeTok{bty=}\StringTok{'L'}\NormalTok{)}
  \KeywordTok{par}\NormalTok{(}\DataTypeTok{xpd =} \OtherTok{TRUE}\NormalTok{)}
  \KeywordTok{legend}\NormalTok{(}\StringTok{"right"}\NormalTok{, }\DataTypeTok{legend =} \NormalTok{(}\KeywordTok{unique}\NormalTok{(PRAassay.m$variable)), }\DataTypeTok{col =} \NormalTok{(}\KeywordTok{unique}\NormalTok{(PRAassay.m$variable)),}\DataTypeTok{pch=}\DecValTok{19}\NormalTok{,}\DataTypeTok{bg=}\StringTok{"white"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{ggplot}\NormalTok{(PRAassay.m, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \NormalTok{PRAassay.m$Time, }\DataTypeTok{y =} \NormalTok{PRAassay.m$value),}\DataTypeTok{color=}\StringTok{"variable"}\NormalTok{) +}\StringTok{ }\KeywordTok{theme_bw}\NormalTok{() }
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{qplot}\NormalTok{(Time,value, }\DataTypeTok{data =} \NormalTok{PRAassay.m,}\DataTypeTok{colour=}\NormalTok{variable)+}\KeywordTok{theme_bw}\NormalTok{()+}\StringTok{ }\KeywordTok{theme}\NormalTok{(}\DataTypeTok{legend.position  =}\StringTok{"bottom"}\NormalTok{,}\DataTypeTok{legend.text=}\KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size=}\DecValTok{10}\NormalTok{),}\DataTypeTok{plot.title =} \KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size =} \DecValTok{14}\NormalTok{, }\DataTypeTok{face =} \StringTok{"bold"}\NormalTok{),}\DataTypeTok{text =} \KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size =} \DecValTok{12}\NormalTok{),}\DataTypeTok{axis.title =} \KeywordTok{element_text}\NormalTok{(}\DataTypeTok{face=}\StringTok{"bold"}\NormalTok{),}\DataTypeTok{axis.text.x=}\KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size =} \DecValTok{10}\NormalTok{),}\DataTypeTok{axis.text.y=}\KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size =} \DecValTok{7}\NormalTok{))+}\KeywordTok{facet_grid}\NormalTok{(PRAassay.m$variable~.,}\DataTypeTok{scales=}\StringTok{"free"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
   \NormalTok{##pendientes (slopes) of linear part of the curve}
  \CommentTok{# time when TrpF is added}
  \NormalTok{cuttime<-}\FloatTok{344.2}
  \NormalTok{maxtime=}\DecValTok{620}
  
  \NormalTok{PRAassay2Time<-tablePRA[}\KeywordTok{which}\NormalTok{(tablePRA$Time >=}\StringTok{ }\NormalTok{cuttime &}\StringTok{ }\NormalTok{tablePRA$Time <=}\StringTok{ }\NormalTok{maxtime),]}
  
  \NormalTok{## removi columnas que se salian del margen de medicion}
  \CommentTok{#PRAassay2Time<-PRAassay2Time[, !(colnames(PRAassay2Time) %in% c("C62uM"))]}
  
  \CommentTok{# vector with slopes for each dataset}
  \NormalTok{V0 <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(PRAassay2Time, }\DecValTok{2}\NormalTok{, function(x) }\KeywordTok{coefficients}\NormalTok{(}\KeywordTok{lm}\NormalTok{(x ~}\StringTok{ }\NormalTok{PRAassay2Time$Time,}\DataTypeTok{na.action=}\NormalTok{na.omit))[}\DecValTok{2}\NormalTok{])}
  
  \NormalTok{PRAassay2Time.m <-}\StringTok{ }\KeywordTok{melt}\NormalTok{(PRAassay2Time,}\DataTypeTok{id=}\StringTok{"Time"}\NormalTok{)}
  \KeywordTok{plot}\NormalTok{(PRAassay2Time.m$Time, PRAassay2Time.m$value, }\DataTypeTok{col=}\NormalTok{PRAassay2Time.m$variable,}\DataTypeTok{xlab=}\StringTok{"Time"}\NormalTok{, }\DataTypeTok{ylab=}\StringTok{"URF PRA uM"}\NormalTok{, }\DataTypeTok{pch=}\DecValTok{19}\NormalTok{, }\DataTypeTok{xlim=}\KeywordTok{c}\NormalTok{(cuttime}\DecValTok{-50}\NormalTok{,maxtime}\DecValTok{+50}\NormalTok{),}\DataTypeTok{bty=}\StringTok{'L'}\NormalTok{)}
  
  \NormalTok{V1 <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(PRAassay2Time[,}\DecValTok{2}\NormalTok{:}\KeywordTok{ncol}\NormalTok{(PRAassay2Time)], }\DecValTok{2}\NormalTok{, function(x) }\KeywordTok{coefficients}\NormalTok{(}\KeywordTok{lm}\NormalTok{(x ~}\StringTok{ }\NormalTok{PRAassay2Time$Time,}\DataTypeTok{na.action=}\NormalTok{na.omit)))}
  \KeywordTok{apply}\NormalTok{(V1, }\DecValTok{2}\NormalTok{, function(x) \{}\KeywordTok{abline}\NormalTok{(x, }\DataTypeTok{col =} \NormalTok{PRAassay2Time.m$variable)  \})}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Highlighting}[]
  \KeywordTok{par}\NormalTok{(}\DataTypeTok{xpd =} \OtherTok{TRUE}\NormalTok{)}
  \NormalTok{slopeText2=}\StringTok{"m ="}
  \NormalTok{slopeText<-}\KeywordTok{paste}\NormalTok{(slopeText2,}\KeywordTok{rev}\NormalTok{(}\KeywordTok{round}\NormalTok{(V0[-}\DecValTok{1}\NormalTok{],}\DataTypeTok{digits=}\DecValTok{6}\NormalTok{)),}\KeywordTok{rev}\NormalTok{(}\KeywordTok{names}\NormalTok{(V0[-}\DecValTok{1}\NormalTok{])))}
  \KeywordTok{legend}\NormalTok{(}\DecValTok{450}\NormalTok{,}\DecValTok{100}\NormalTok{, }\DataTypeTok{legend =} \NormalTok{slopeText, }\DataTypeTok{col =} \KeywordTok{rev}\NormalTok{(}\KeywordTok{unique}\NormalTok{(PRAassay2Time.m$variable)), }\DataTypeTok{pch=}\DecValTok{19}\NormalTok{,}\DataTypeTok{box.lwd=}\DecValTok{0}\NormalTok{, }\DataTypeTok{bg=}\StringTok{"white"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{## get relevant velocities and get correspondent concentrations}
  \NormalTok{## V0 or slopes are on [PRA]uM/s }
  \CommentTok{# slopes stores the slope, with TRUE , FALSE the desired enzyme can be selected, One more than one is tested on a plate always check if it is the desired one.}
  \NormalTok{slopes=(}\KeywordTok{round}\NormalTok{(V0[-}\DecValTok{1}\NormalTok{],}\DataTypeTok{digits=}\DecValTok{6}\NormalTok{))[}\KeywordTok{c}\NormalTok{(}\OtherTok{TRUE}\NormalTok{)]}
  \NormalTok{concentration<-}\KeywordTok{as.matrix}\NormalTok{(PRAassay2Time[}\DecValTok{1}\NormalTok{,}\KeywordTok{names}\NormalTok{(slopes)])}
  \CommentTok{#slopes}
  \CommentTok{#concentration}
  \NormalTok{slopes=}\KeywordTok{as.matrix}\NormalTok{((}\KeywordTok{round}\NormalTok{(V0[-}\DecValTok{1}\NormalTok{],}\DataTypeTok{digits=}\DecValTok{6}\NormalTok{))[}\KeywordTok{c}\NormalTok{(}\OtherTok{TRUE}\NormalTok{)])}
  \KeywordTok{row.names}\NormalTok{(slopes) <-}\StringTok{ }\OtherTok{NULL} 
  \NormalTok{slopes<-}\KeywordTok{c}\NormalTok{(slopes)## plot on r base}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \CommentTok{# in concentration I stored the inital substrate concentration  }
  \KeywordTok{row.names}\NormalTok{(concentration) <-}\StringTok{ }\OtherTok{NULL} 
  \NormalTok{concentration<-}\KeywordTok{c}\NormalTok{(concentration)}
  \NormalTok{slopes}
  \end{Highlighting}
  \end{Shaded}
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \NormalTok{## interpolar michaelis-menden}
  
  \CommentTok{#https://rpubs.com/RomanL/6752}
  \CommentTok{#https://davetang.org/muse/2013/05/17/fitting-a-michaelis-mentens-curve-using/}
  \CommentTok{#S <-concentration}
  \NormalTok{## from highest to lower concentration}
  \NormalTok{S_ProFAR <-}\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{25}\NormalTok{,}\DecValTok{50}\NormalTok{,}\DecValTok{75}\NormalTok{,}\DecValTok{100}\NormalTok{,}\DecValTok{0}\NormalTok{,}\DecValTok{50}\NormalTok{,}\DecValTok{100}\NormalTok{)}
  \NormalTok{v<-slopes}
  \CommentTok{#v<-slopes[-length(slopes)]}
  \CommentTok{#v <-slopes[c(TRUE,FALSE)]}
  \NormalTok{v<--}\DecValTok{1}\NormalTok{*v}
  \NormalTok{S_ProFAR}
  \NormalTok{v}
  \NormalTok{mm <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(S_ProFAR,v)                                                                                 }
  \NormalTok{model.drm <-}\StringTok{ }\KeywordTok{drm}\NormalTok{(v ~}\StringTok{ }\NormalTok{S, }\DataTypeTok{data =} \NormalTok{mm, }\DataTypeTok{fct =} \KeywordTok{MM.2}\NormalTok{())}
  \KeywordTok{summary}\NormalTok{(model.drm)}
  \NormalTok{## first value equal km}
  \NormalTok{## second value = vm}
  \NormalTok{Km=}\KeywordTok{coefficients}\NormalTok{(model.drm)[}\DecValTok{1}\NormalTok{]}
  \NormalTok{Vmax=}\DecValTok{2}\NormalTok{*}\KeywordTok{coefficients}\NormalTok{(model.drm)[}\DecValTok{2}\NormalTok{]}
  \NormalTok{Enzyme=}\FloatTok{2.5} \CommentTok{#2.5uM}
  \NormalTok{Kcat=Vmax*Enzyme  }
  \NormalTok{Km}
  \NormalTok{Vmax}
  \NormalTok{Kcat}
  \NormalTok{mml <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{S =} \KeywordTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(mm$S), }\DataTypeTok{length.out =} \DecValTok{100}\NormalTok{))}
  \NormalTok{mml$v <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(model.drm, }\DataTypeTok{newdata =} \NormalTok{mml)}
  
  
  \NormalTok{## plot on r base}
  \KeywordTok{plot}\NormalTok{(mm,}\DataTypeTok{log=}\StringTok{''}\NormalTok{,}\DataTypeTok{xlim=}\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\KeywordTok{max}\NormalTok{(mm$S)), }\DataTypeTok{ylim=}\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\KeywordTok{max}\NormalTok{(mm$v)), }\DataTypeTok{xlab=}\StringTok{"Reads"}\NormalTok{, }\DataTypeTok{ylab=}\StringTok{"Transcripts"}\NormalTok{)}
  
  \NormalTok{##plot on ggplot}
  \KeywordTok{ggplot}\NormalTok{(mm, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \NormalTok{S, }\DataTypeTok{y =} \NormalTok{v)) +}\StringTok{ }\KeywordTok{theme_bw}\NormalTok{() +}\StringTok{ }\KeywordTok{xlab}\NormalTok{(}\StringTok{"Concentration [uM]"}\NormalTok{) +}\StringTok{ }\KeywordTok{ylab}\NormalTok{(}\StringTok{"Speed [d[PRA]uM/s]"}\NormalTok{) +}\StringTok{ }\KeywordTok{ggtitle}\NormalTok{(}\StringTok{"Michaelis-Menten kinetics"}\NormalTok{) +}\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{alpha =} \FloatTok{0.5}\NormalTok{) +}\StringTok{ }\KeywordTok{geom_line}\NormalTok{(}\DataTypeTok{data =} \NormalTok{mml, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \NormalTok{S, }\DataTypeTok{y =} \NormalTok{v), }\DataTypeTok{colour =} \StringTok{"red"}\NormalTok{)}
  
  \KeywordTok{ggsave}\NormalTok{(}\StringTok{"mm.pdf"}\NormalTok{, }\DataTypeTok{width =} \DecValTok{6}\NormalTok{, }\DataTypeTok{height =} \DecValTok{4}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  ...
  166. Henn-Sax M, Thoma R, Schmidt S, Hennig M, Kirschner K, Sterner R.
  ...
  167. Sterner R, Kleemann GR, Szadkowski H, Lustig A, Kirschner K, Hennig
  ...
  168. Due AV, Kuper J, Geerlof A, Kries JP von, Wilmanns M. Bisubstrate
  ...
  169. Noda-García L, Camacho-Zarco AR, Verdel-Aranda K, Wright H, Soberón
  ...
  170. Gao Y, Xu G, Wu P, Liu J, Cai Y-s, Deng Z, et al. Biosynthesis of
  ...
  171. Marks DS, Colwell LJ, Sheridan R, Hopf TA, Pagnani A, Zecchina R,
  ...
  172. Hopf TA, Green AG, Schubert B, Mersmann S, Schärfe CPI, Ingraham
  ...
  173. Chertkov O, Sikorski J, Nolan M, Lapidus A, Lucas S, Del Rio TG, et
  ...
  174. Pukall R, Gehrich-Schröter G, Lapidus A, Nolan M, Glavina Del Rio
  ...
  175. Angel E. Interactive computer graphics : A top-down approach with
  opengl. Boston, MA: Addison Wesley Longman; 2000.
  
  \hypertarget{ref-angel2001}{}
  176. Angel E. Batch-file computer graphics : A bottom-up approach with
  quicktime. Boston, MA: Wesley Addison Longman; 2001.
  
  \hypertarget{ref-angel2002a}{}
  177. Angel E. Test second book by angel. Boston, MA: Wesley Addison
  Longman; 2001.
