...
=======
  ...
  \CommentTok{#con=url("http://bowtie-bio.sourceforge.net/recount/ExpressionSets/bodymap_eset.RData")}
  \CommentTok{#load(file=con)}
  \CommentTok{#close(con)}
  \CommentTok{#bm = bodymap.eset}
  \CommentTok{#bm}
  \CommentTok{#exp_data = exprs(bm)}
  \CommentTok{#dim(exp_data)}
  \CommentTok{#head(exp_data,n=5)}
  \CommentTok{#pheno_data = pData(bm)}
  \CommentTok{#dim(pheno_data)}
  \CommentTok{#head(pheno_data)}
  \CommentTok{#feature_data = fData(bm)}
  \CommentTok{#dim(fData(bodymap.eset))}
  \CommentTok{#fData(bodymap.eset)[1:10,,1]}
  \CommentTok{#devtools::session_info()}
  \end{Highlighting}
  \end{Shaded}
...
=======
  ...
  enlargment (Larsson, Nylander, \& Bergman, 2011) by the analysis of 58
  Cyanobacterial genomes concludes ancestor of cyanobacteria had a genome
  size of approx. 4.5 Mbp. Cyanobacteria produces natural products as
  pigments and toxins (Whitton, 2012) Example of a PriA cluster
