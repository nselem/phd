...
% Everything below added by CII
  ...
  instalación. El contenedor docker implementa EVcouplings python
  framework {[}\protect\hyperlink{ref-hopf_evcouplings_2019}{174}{]} que
  ...
  coelicolor} (identificador de Uniprot HIS4\_STRCO). Las secuencias para
  la alineación se recuperaron automáticamente de Uniprot, otros
  parámetros se dejaron con la configuración inicial del archivo de
  configuración de EVCouplings. Como resultado se modeló la estructura de
  PriA utilizando los acoplamientos de sus aminoácidos. Una comparación de
  la estructura lograda con una estructura cristalográfica es mostrada en
  la \autoref{fig:CouplingsFoldingPriA}. El RMSD es una técnica usual de
  comparaci\{on de estructuras. EL RSMD entre estas dos proteínas es de
  3.362 \(\r{A}\), es incluso mejor que el 3.730 entre la 1VZW y la 4TX9
  de \emph{Streptomyces sviceus ATCC 29083}. Como las estructuras tienen
  una resolución de 1-3 A° un RMSD es aceptable. Sin embargo los RMSD
  Entre otras estructuras cristalográficas de Streptomyces coelicolor son
  menores a 1. 2.7--4.8 Å Error de EVCouplings 0.449 2VEP 1VZW
  
  Así pues, la estructura reconstruida es parecida, pero para obtener
  mejor definición es posible que se deba refinar la selección de las
  secuencias del alineamiento, diferenciando entre secuencias conocidas de
  PriA, subHisA, PriB y subTrpF.
  ...
  \KeywordTok{kable}\NormalTok{(table,  }\DataTypeTok{caption =} \StringTok{"Estructuras cristalográficas disponibles de PriA y familias relacionadas }\CharTok{\textbackslash{}\textbackslash{}}\StringTok{label\{tab:EnzymePDB\}"}\NormalTok{,}\DataTypeTok{caption.short =} \StringTok{"Estructuras cristalográficas disponibles de PriA y familias relacionadas"}\NormalTok{)}\OperatorTok{%>%}\StringTok{  }\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options=}\StringTok{"scale_down"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
