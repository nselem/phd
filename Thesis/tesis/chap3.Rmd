...
# Merging heatplot and axonomy table into one table
...
#small$RastId<-factor(unique(small$RastId), c("390212","389007","388991","388992"))
#small$order <- c("1", "2", "3","4")
#small
#small$RastId<-factor(unique(small$RastId),small$RastId[order(unique(small$order))])
#small$RastId
...
## Expansion function IDea I could Also do a reduction function..
expansion <- function(x) {
  mean = mean(x) 
  sd = sd(x) 
  expansion=mean+sd
  reduction=mean-sd
  cutoff=c(expansion,reduction)
  return(cutoff)
  }


## For all families print what means an Archaeal expansion
for (family in unique(small$variable)){
    ## Defining what is an expansion
    fam_cutoff<-expansion(small$value[which(small$variable==family)])
    print(paste("For family", family,"expansion means",fam_cutoff[1]))
    print(paste("For family", family,"reduction means",fam_cutoff[2]))
    
    ## Finding wich values on that families are expansions
    ## that means wich genomes ahs an expansion on that family
    for(smallvalue in small$value[which(small$variable==family)]){
      if(smallvalue>fam_cutoff[1]){
        print(paste("This is an expansion",smallvalue,">",fam_cutoff[1]))
      }
      else if(smallvalue<fam_cutoff[2]){
        print(paste("This is a reduction",smallvalue,"<",fam_cutoff[2]))
        }
      else{
        print (paste ("Among average",smallvalue,"<",fam_cutoff[1]))}
    }
    fam_cutoff<-NULL
}

## HAcer una estructura de datos, expansion, reduction normal
