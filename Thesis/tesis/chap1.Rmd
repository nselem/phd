...
```{r ArbolOrthocore, results = "asis",message=FALSE,echo=FALSE}
# Arbol de orthocore
label(path = "chapter1/ArbolOrthocore.png", caption = "ArbolOrthocore",label = "ArbolOrthocore", type = "figure")
```

<!---
Tabla de orthocore 
--->
```{r EjemploMicrobioma, results = "asis"}
table <- read.csv("chapter1/MicrobiomaMetadata.csv", row.names = 1,sep="\t")
kable(table,  caption = "Ejemplo de Microbioma \\label{tab:Ejemplo de Microbioma}",caption.short = "Microbioma de tomate ")
```
...
```{r ArbolClavisual, results = "asis",message=FALSE,echo=FALSE}
#Arbol de clavisual
label(path = "chapter1/ArbolClavisual.png", caption = "ArbolClavisual",label = "ArbolClavisual", type = "figure")
```
...
y <- c(2684,2504,2161,1867,1613,1217,745,432,320)
 plot(x,y, xlab="Número de genomas ", ylab="Número de Genes en el Core genome", main="Core Genome de organismos del Microbioma del tomate", ylim=c(0,4000), xlim=c(0,12), pch=15, col="blue") 
```
