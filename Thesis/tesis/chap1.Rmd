...
El pseudocore consiste en utilizar información previa específica del core de un linaje para obtener los genes de  y la metodología está depositada en github integrada en el repositorio [clavigenomics](https://github.com/nselem/clavigenomics). El blast fue optimizado cambiando hacer un blast todos contra todos por archivos genómicos individuales genomai_vs_genomaj.blast que luego son concatenandos según se necesiten.  

Los porcentajes de genomas son diferentes porque al no bastar los best bidireccional hits conservados, todo el pangenoma es decir todos los genes contenidos en los genomas del grupo de interés necesitan ser clasificados por familias, para de ahi obtener las familias que tienen presencia en un porcentaje %p y ausencia en un porcentaje a% del grupo externo. Estos perfiles fueron desarrollados para Clavisual `r ref(label = "Clavisual", type = "figure")` utilizando FasthOrtho para clasificar las familias y de ahi obtener los grupos. Con ellos se consiguieron marcadores para Kurtobacterium.  
...
Tomando otros siete genomas del género _Clavibacter_, utilizando OrthoVenn tenemos la misma observación, el número de familias de genes agregadas al adicionar genomas, es después de siete genomas casi tan grande como su core `r ref(label = "Venn-Chart", type = "figure")`. 
