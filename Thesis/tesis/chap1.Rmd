...
label(path = "chapter1/coreWiki.png", caption = "Orthocore obtiene familias de genes conservados en un linaje genómico", label = "Orthocore calcula las familias génicas comunes de un linaje genómico. Después de un proceso de filtrado, alineamiento y curación concatena estas familias y entrega una reconstrucción filogenética." , scale=".42", type = "figure")
```
...
La organización filogenética de un linaje genómico permite la observación de pérdida y ganancia de familias génicas en organismos cercanos. Si los organismos están desordenados es difícil apreciar la dinámica genómica de aparición-desaparición de copias extra en una familia génica. Ordenar los genomas de un linaje facilita apreciar cambios en el número de copias de una familia. Esto es relevante en el marco de esta tesis ya que cambios en los perfiles de promiscuidad podrían estar relacionados a copias extras de organismos cercanos. Orthocore es un algoritmo facilitador de la organización filogenética de organismo de un linaje.  
...
label(path = "chapter1/clv.png", caption = " XXX",scale=".7",label = "Marcadores-Clavibacter", type = "figure")
```
...
label(path = "chapter1/CoreGenomeMicrobioma.png",scale=".6", caption = " X X",label = "Marcadores-Clavibacter", type = "figure")
```
